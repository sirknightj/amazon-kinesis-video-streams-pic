...
TEST_F(EnvironmentFunctionalityTest, IsEnvVarSetDefaultTrueUnsetReturnsTrue)
{
    ...
    EXPECT_TRUE(isEnvVarSet((PCHAR) envVar.c_str(), TRUE));

    // Unset var with default=FALSE should return FALSE.
    EXPECT_FALSE(isEnvVarSet((PCHAR) envVar.c_str(), FALSE));
}

TEST_F(EnvironmentFunctionalityTest, IsEnvVarSetExplicitDisable)
{
    std::string envVar = "KVS_TEST_ENV_VAR_SET";
    ...
    for (std::string val : disabledValues) {
        ...
        EXPECT_FALSE(isEnvVarSet((PCHAR) envVar.c_str(), TRUE));
    }
...
TEST_F(EnvironmentFunctionalityTest, IsEnvVarSetExplicitEnable)
{
    std::string envVar = "KVS_TEST_ENV_VAR_SET";
    ...
    for (std::string val : enabledValues) {
        ...
        EXPECT_TRUE(isEnvVarSet((PCHAR) envVar.c_str(), FALSE));
    }
...
TEST_F(EnvironmentFunctionalityTest, IsEnvVarSetUnrecognizedValueReturnsDefault)
{
    std::string envVar = "KVS_TEST_ENV_VAR_SET";
...
TEST_F(EnvironmentFunctionalityTest, IsEnvVarSetNullAndEmpty)
{
    EXPECT_TRUE(isEnvVarSet((PCHAR) NULL, TRUE));
    EXPECT_FALSE(isEnvVarSet((PCHAR) NULL, FALSE));
    EXPECT_TRUE(isEnvVarSet((PCHAR) "", TRUE));
    EXPECT_FALSE(isEnvVarSet((PCHAR) "", FALSE));
}
